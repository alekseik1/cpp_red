...
ostream &operator<<(ostream& os, const HttpRequest &request) {
    os << request.method << ' ' << request.uri << ' ' << request.protocol;
    return os;
}
...
void testParseRequest() {
    const map<string_view, HttpRequest> expected_parsed {
            {"POST /order HTTP/1.1", {"POST", "/order", "HTTP/1.1"}},
            {"POST /product HTTP/1.1", {"POST", "/product", "HTTP/1.1"}}
    };
    for (const auto& pair : expected_parsed) ASSERT_EQUAL(ParseRequest(pair.first), pair.second);
}
