...
public:
    ...
    void PushFront(const T &value) { _front.push_back(value); }

    void PushBack(const T &value) { _back.push_back(value); }

    const T &Front() const {
        if (!_front.empty()) return _front.back();
        return _back[0];
    }
    ...
    const T &Back() const
    {
        if (!_back.empty()) return _back.back();
        return _front[0];
    }
    ...
    const T &At(size_t i) const {
        if (i < _front.size()) return _front.at(_front.size() - i - 1);
    ...
    const T &operator[](size_t i) const {
        if (i < _front.size()) return _front[_front.size() - i - 1];
