...
public:
  ...
  AirportCounter(TIterator begin, TIterator end) {
      ...
      for(auto it = begin; it != end; ++it) {
          auto airport_id = static_cast<uint32_t>(*it);
          ++counter[airport_id];
      }
  }
  ...
  size_t Get(TAirport airport) const {
      auto airport_id = static_cast<uint32_t>(airport);
      return counter.at(airport_id);
  }
  ...
  Items GetItems() const {
      array<Item, static_cast<size_t>(TAirport::Last_)> result;
      for(size_t i=0; i < counter.size(); ++i) {
          result[i] = {static_cast<TAirport>(i), counter[i]};
      }
      return result;
  }

private:
    array<size_t, static_cast<uint32_t>(TAirport::Last_)> counter;
};
